...
| --- | --- | --- |
| `gsm8k` | Grade-school math reasoning. Plain GSM8K is kept as a single-turn sanity check, while GSM8K + Tool is the minimal `ToolEnv + BaseTool` example with dynamic prompts and `calc_gsm8k_reward`. | `data_preprocess/process_gsm8k.py`, `data_preprocess/process_gsm8k_agent.py`, `env/gsm8k_tool_env.py`, `tools.py` |
