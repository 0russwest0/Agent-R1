...
# 1) train.parquet / validation.parquet for verl/agent_r1 RLHFDataset (prompt + reward_model + data_source)
...
#   python recipe/hotpotqa/prepare_hotpotqa_agent_r1.py \
...
def _cross_eval_row_to_agent_r1(
    ...
    """Convert one cross-eval QA example to the HotpotQA Agent-R1 row schema.
...
def _write_cross_eval_parquets(
    ...
    """Write cross-eval validation parquets in HotpotQA Agent-R1 schema.
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Prepare HotpotQA for Agent-R1 / verl RL training.")
    parser.add_argument(
        "--output_dir",
        type=str,
